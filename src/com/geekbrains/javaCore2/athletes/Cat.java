...
public class Cat extends Player implements Runable, Jumpable {
    ...
    public void jump(Wall wall) {
        if (jumpHeight>wall.getHeight()) {
            System.out.println("Кот " + name + " перепрыгнул стену высотой " + wall.getHeight() + " метров");
        } else {
            System.out.println("Коту " + name + " не удалось перепрыгнуть стену высотой " + wall.getHeight() + " метров");
            passed = false;
        }
    ...
    public void run(Treadmill treadmill) {
        if (runDistance > treadmill.getDistance()) {
            System.out.println("Кот " + name + " успешно пробежал " + treadmill.getDistance() + " метров");
        } else {
            System.out.println("Коту " + name + " не удалось пробежать " + treadmill.getDistance() + " метров");
            passed = false;
        }
